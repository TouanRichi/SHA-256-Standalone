...
{   
    ...
    else if(argumentCount >= 1)
    {
        ...
        else
        {
            ...
            double b = elapsed_first * CPU_FREQ;
            printf("Số chu kỳ CPU ước tính: %.0f cycles\n", b);
            // VÒNG LẶP TÍNH THỜI GIAN TRUNG BÌNH - REMOVED to fix segmentation fault
            // The file is already closed after calculateHash, so can't reuse
            printf("Note: Multiple runs disabled to prevent file access errors.\n");
